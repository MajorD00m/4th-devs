...
# PROMPT do budowy narzędzia

Potrzebuję w ramach mojego agenta AI, opracować wydajną (szybką i tanią) analizę logów z czujników (chodzi o wykrywanie ANOMALI).
Czy to zrobić jako tool używający agenta/model LLM (jak tool analyze_local_image) czy może jako subagenta?
Logi są w postaci plików JSON, jeden plik to jeden odczyt z czujnika, gdzie jeden czujnik może mieć wiele sensorów.
Tych logów jest bardzo dużo.
Format pojedynczego pliku to:
Każdy czujnik zwraca dane w poniższym formacie:
Format danych w pojedynczym pliku JSON:
...
Typy sensorów: humidity, pressure, temperature, voltage, water
...
Nazwa zadania to: evaluation
Odpowiedź wysyłasz do Centrali w formacie jak poniżej:
{ "recheck": ["0001","0002","0003", "..."] }

Dane z sensorów pobierzesz tutaj: https://hub.ag3nts.org/dane/sensors.zip
Dane wysyłasz do centrali jako tablicę JSON (jak wyżej) zawierającą identyfikatory.
Pobierz plik, rozpakuj, i zaproponuj jak planujesz podejśc do analizy tych logów?

Czy masz dostępne jakies narzędzie do analizy logów sensorów?

## WYNIK
Trwało długo z FLEX ale zadziałało
